...
DEFAULT_POSTSEASON_MONTHS = "9-12"
...
def build_argument_parser() -> argparse.ArgumentParser:
	...
	parser.add_argument(
		...
		default=POSTSEASON_SERIES_IDS,
		help="Comma-separated postseason series IDs used by KBO.",
	)
	...
	parser.add_argument(
		"--refresh-postseason-history",
		action="store_true",
		help="Rebuild only the frozen historical postseason workbook before writing outputs.",
	)
	...
	parser.add_argument(
		"--postseason-months",
		default=DEFAULT_POSTSEASON_MONTHS,
		help="Month range used for postseason crawls.",
	)
...
def is_generated_data_path(path: str) -> bool:
	...
	return (
		name in {
			"kbo_schedule.xlsx",
			"kbo_team_sheets.xlsx",
			"kbo_postseason_schedule.xlsx",
			"kbo_postseason_team_sheets.xlsx",
		}
		or name.startswith("kbo_schedule_history_")
		or name.startswith("kbo_schedule_")
		or name.startswith("kbo_team_sheets_")
		or name.startswith("kbo_postseason_history_")
	)
...
def history_cache_path(years: list[int], output_dir: Path, postseason: bool = False) -> Path:
	prefix = "kbo_postseason_history" if postseason else "kbo_schedule_history"
	return output_dir / f"{prefix}_{history_cache_label(years)}.xlsx"
...
def read_history_schedule(
	...
	if "competition_type" not in frame.columns:
		...
		frame["competition_type"] = frame["competition_type"].fillna(competition_type)
...
def raw_month_paths(
	...
	root = repo_root()
	if competition_type == "postseason":
		return [
			root / "data" / "raw" / str(year) / "postseason" / f"postseason_{year}_{month:02d}.json"
			for year in years
			for month in months
		]
	return [
...
def main() -> int:
	...
	postseason_months = parse_months(args.postseason_months)
	...
	postseason_paths = postseason_output_paths(years[-1])
	scoreboard_cache_by_year = read_cached_scoreboard_details(paths.xlsx_path, years) if args.daily else {}
	postseason_cache_by_year = (
		read_cached_scoreboard_details(postseason_paths.xlsx_path, years) if args.daily else {}
	)
	postseason_history_cache_by_year = read_cached_scoreboard_details(
		postseason_paths.xlsx_path,
		history_years,
	)
	requested_history_workbook_path = history_cache_path(history_years, paths.xlsx_path.parent)
	requested_postseason_history_path = history_cache_path(
		history_years,
		postseason_paths.xlsx_path.parent,
		postseason=True,
	)
	history_workbook_path: Path | None = None
	history_frame: pd.DataFrame | None = None
	postseason_history_workbook_path: Path | None = None
	postseason_history_frame: pd.DataFrame | None = None
	...
	if args.refresh_history or args.refresh_postseason_history:
		postseason_history_records = crawl_years(
			history_years,
			postseason_months,
			series_ids=args.postseason_series_ids,
			team_id=args.team_id,
			scoreboard_cache_by_year=postseason_history_cache_by_year,
			competition_type="postseason",
		)
		postseason_history_frame = build_schedule_dataframe(postseason_history_records)
		postseason_history_workbook_path = write_schedule_workbook(
			postseason_history_frame,
			requested_postseason_history_path,
		)
	elif args.daily:
		try:
			postseason_history_frame = read_history_schedule(
				requested_postseason_history_path,
				history_years,
				competition_type="postseason",
			)
			postseason_history_workbook_path = requested_postseason_history_path
		except (FileNotFoundError, ValueError) as exc:
			parser.error(
				f"{exc} Run once with --refresh-postseason-history before using --daily."
			)
	...
	postseason_live_records = crawl_years(
		years,
		postseason_months,
		series_ids=args.postseason_series_ids,
		team_id=args.team_id,
		scoreboard_cache_by_year=postseason_cache_by_year,
		competition_type="postseason",
	)
	postseason_live_frame = build_schedule_dataframe(postseason_live_records)
	if (
		postseason_history_frame is not None
		and not postseason_history_frame.empty
		and "season_year" in postseason_history_frame.columns
	):
		postseason_history_frame = postseason_history_frame[
			~postseason_history_frame["season_year"].isin(sorted(set(years)))
		].copy()
	postseason_frame = combine_schedule_frames(postseason_history_frame, postseason_live_frame)
	postseason_workbook_path = write_schedule_workbook(postseason_frame, postseason_paths.xlsx_path)
	postseason_team_frame = build_team_sheet_rows(postseason_frame)
	requested_postseason_team_path = postseason_paths.xlsx_path.with_name("kbo_postseason_team_sheets.xlsx")
	postseason_team_workbook_path = write_team_workbook(
		postseason_team_frame,
		requested_postseason_team_path,
	)
	...
	if postseason_history_frame is not None:
		print(f"Frozen postseason history rows: {len(postseason_history_frame)}")
		print(f"Live postseason crawl rows: {len(postseason_live_frame)}")
	print(f"Saved {len(postseason_frame)} postseason games")
	...
	if postseason_history_workbook_path is not None:
		print(f"Frozen postseason history workbook: {postseason_history_workbook_path}")
		if postseason_history_workbook_path != requested_postseason_history_path:
			print(f"Original frozen postseason history workbook was locked: {requested_postseason_history_path}")
	...
	print(f"Postseason workbook: {postseason_workbook_path}")
	if postseason_workbook_path != postseason_paths.xlsx_path:
		print(f"Original postseason workbook was locked: {postseason_paths.xlsx_path}")
	print(f"Postseason team workbook: {postseason_team_workbook_path}")
	if postseason_team_workbook_path != requested_postseason_team_path:
		print(f"Original postseason team workbook was locked: {requested_postseason_team_path}")
	...
	if args.push:
		...
		postseason_history_locked = (
			(args.refresh_history or args.refresh_postseason_history)
			and postseason_history_workbook_path != requested_postseason_history_path
		)
		canonical_workbook_locked = any(
			(
				season_workbook_path != paths.xlsx_path,
				team_workbook_path != requested_team_workbook_path,
				postseason_workbook_path != postseason_paths.xlsx_path,
				postseason_team_workbook_path != requested_postseason_team_path,
				history_workbook_locked,
				postseason_history_locked,
			)
		)
		if canonical_workbook_locked:
			...
			postseason_raw_month_files = raw_month_paths(
				years,
				postseason_months,
				competition_type="postseason",
			)
			...
			postseason_history_output_files: list[Path] = []
			if args.refresh_history or args.refresh_postseason_history:
				postseason_history_output_files = [
					requested_postseason_history_path,
					*raw_month_paths(
						history_years,
						postseason_months,
